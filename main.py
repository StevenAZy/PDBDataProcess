...
if __name__ == "__main__":
    ...
    for i in range(10):
        t = threading.Thread(
            target=download_complexs,
            args=(DOWNLOAD_URL, complex_list[i], PDB_SAVE_PATH, redis_conn),
        )
        t.start()
    ...
    for i in range(10):
        t = threading.Thread(
            target=split_complex,
            args=(PDB_SAVE_PATH, SPLIT_PDB_SAVE_PATH, downloaded_complexs_lsit[i]),
        )
        t.start()
    ...
    for i in range(len(remove_heteroatom_complexs)):
        remove_heteroatom_complexs_lsit[i % 10].append(
            remove_heteroatom_complexs[i].split(".")[0]
        )

    for i in range(10):
        t = threading.Thread(
            target=remove_heteroatom,
            args=(
                SPLIT_PDB_SAVE_PATH,
                NOHETEROATOM_PDB_SAVE_PATH,
                remove_heteroatom_complexs_lsit[i],
            ),
        )
        t.start()
